...
watch(sortTitle, (val) => {
  if (val !== 'none') sortDifficulty.value = 'none'
})
...
const getTitle = (row) => {
  ...
  return (row.problem_name || row.title || proposed.problem_name || proposed.title || '').toLowerCase()
}

const getDifficulty = (row) => {
  const proposed = getProposed(row)
  return (row.difficulty || proposed.difficulty || '').toString().toLowerCase()
}

const filteredRows = computed(() => {
  const t1 = searchTitle.value.toLowerCase()
  const t2 = searchDifficulty.value.toLowerCase()

  let filtered = (props.rows || []).filter((row) => {
    const title = getTitle(row)
    const diff = getDifficulty(row)
    const okTitle = t1 ? title.includes(t1) : true
    const okDiff = t2 ? diff.includes(t2) : true
    return okTitle && okDiff
  })

  // Apply sorting similar to admin pending questions table
  if (sortDifficulty.value !== 'none') {
    filtered = filtered.slice().sort((a, b) => {
      const aVal = getDifficulty(a)
      const bVal = getDifficulty(b)
...
const onApprove = (row) => emit('approve-item', { approval_id: row.id || row.approval_id, content_type: row.content_type || 'problem', row })
const onDeny = (row) => emit('deny-item', { approval_id: row.id || row.approval_id, content_type: row.content_type || 'problem', row })
const onView = (row) => emit('view-item', row)
</script>

<template>
  <TableList class="solo-table" :column_slot="['appr_title','appr_diff','appr_submitted','appr_status','appr_action']">
    <template #appr_title>
      <div>
        <p>Problem Title</p>
        <SearchBarAndSort
    ...
    <template #content>
      <tr class="solo-table-row" v-for="(row, idx) in filteredRows" :key="row.id || idx">
        <td>{{ row.problem_name || row.title || ((typeof row.proposed_data === 'string') ? (JSON.parse(row.proposed_data || '{}').problem_name || JSON.parse(row.proposed_data || '{}').title) : (row.proposed_data && (row.proposed_data.problem_name || row.proposed_data.title))) || 'Untitled Problem' }}</td>
        <td>{{ row.difficulty || ((typeof row.proposed_data === 'string') ? (JSON.parse(row.proposed_data || '{}').difficulty) : (row.proposed_data && row.proposed_data.difficulty)) || '—' }}</td>
      ...
      <tr v-if="filteredRows.length === 0">
        <td colspan="5" class="empty">No pending problems</td>
      </tr>
    </template>
  </TableList>
</template>
