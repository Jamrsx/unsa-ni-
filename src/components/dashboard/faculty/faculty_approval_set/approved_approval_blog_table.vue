...
const filteredRows = computed(() => {
  ...
  let filtered = (props.rows || []).filter((row) => {
    ...
    const title = (row.blog_title || row.title || pd.blog_title || pd.title || '').toLowerCase()
    const author = (row.author || row.blog_author || pd.author || pd.blog_author || row.faculty_username || '').toLowerCase()
    const okTitle = t1 ? title.includes(t1) : true
    const okAuthor = t2 ? author.includes(t2) : true
    return okTitle && okAuthor
  })

  // Apply sorting similar to admin approved blogs table
...
const displayTitle = (row) => {
  const pd = parseProposed(row)
  return row.blog_title || row.title || pd.blog_title || pd.title || 'Untitled Blog'
}

const displayAuthor = (row) => {
  const pd = parseProposed(row)
  return row.author || row.blog_author || pd.author || pd.blog_author || row.faculty_username || '—'
}
...
<template>
  <TableList class="solo-table" :column_slot="['appr_title','appr_author','appr_committed','appr_status','appr_action']">
    <template #appr_title>
      <div>
        <p>Blog Title</p>
        <SearchBarAndSort
    ...
    <template #content>
      ...
      <tr v-if="filteredRows.length === 0">
        <td colspan="5" class="empty">No approved blogs</td>
      </tr>
    </template>
  </TableList>
</template>
